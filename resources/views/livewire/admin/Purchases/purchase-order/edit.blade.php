<div x-data="{
    ...
    calculate() {
        let tempTotal = 0;
        // Validación extra para asegurar que es un array
...
}" @update-products.window="total = parseFloat($event.detail.total)">
    <x-w-card title="Editar Orden de Compra: {{ $serie }} - {{ $correlative }}">
        <form wire:submit.prevent="save" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <x-w-select label="Orden de compra" placeholder="Seleccione una orden de compra"
                    wire:model.live="purchase_order_id" :async-data="['api' => route('api.purchase-orders.index'), 'method' => 'POST']" option-label="name" option-value="id"
                    option-description="description" />
                <div class="col-span-1 md:col-span-2">
                    <x-w-select label="Proveedor" placeholder="Seleccione un proveedor" wire:model="supplier_id"
                        :async-data="['api' => route('api.suppliers.index'), 'method' => 'POST']" option-label="name" option-value="id" />
                </div>
                <div class="col-span-1 md:col-span-2">
                    <x-w-select label="Almacen" placeholder="Seleccione un almacen" wire:model="warehouse_id"
            ...
            <div class="flex flex-col sm:flex-row items-end gap-3">
                <div class="w-full">
                    <x-w-select label="Producto" placeholder="Seleccione un producto" wire:model="product_id"
                        :async-data="['api' => route('api.products.index'), 'method' => 'POST']" option-label="name" option-value="id" />
                </div>
                <x-w-button wire:click="addProduct" spinner="addProduct" primary
            ...
            <div
                ...
                <div class="flex-1">
                    {{-- CORREGIDO: wire:model="observation" (singular) --}}
                    <x-w-input label="Observaciones" wire:model="observation" placeholder="Notas adicionales..." />
                </div>
                <div class="text-right space-y-1">
                    <p class="text-sm text-gray-500 uppercase">Total a pagar</p>
                    <p class="text-3xl font-bold text-primary-600">
                        {{-- Usamos Number() para asegurar que toFixed funcione --}}
                        $<span x-text="Number(total).toFixed(2)"></span>
                        <span class="text-sm font-normal text-gray-400">MXN</span>
                    </p>
                </div>
            </div>
