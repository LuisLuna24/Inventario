...
<head>
    <meta charset="UTF-8">
    <title>Orden de Compra #{{ $purchaseOrder->serie }}-{{ str_pad($purchaseOrder->correlative, 4, '0', STR_PAD_LEFT) }}</title>
    <style>
    ...
    <table class="header-table">
        <tr>
            ...
            <td style="width: 40%; text-align: right;">
                <div style="font-size: 18px; font-weight: bold; color: #e74c3c;">ORDEN DE COMPRA</div>
                <div style="font-size: 14px; margin-top: 5px;"># {{ $purchaseOrder->serie }}-{{ str_pad($purchaseOrder->correlative, 4, '0', STR_PAD_LEFT) }}</div>
                <div style="margin-top: 10px;">
                    <strong>Fecha:</strong> {{ \Carbon\Carbon::parse($purchaseOrder->date)->format('d/m/Y') }}<br>
                    <span style="font-size: 10px; color: #888;">Hora: {{ \Carbon\Carbon::parse($purchaseOrder->created_at)->format('H:i A') }}</span>
                </div>
            </td>
        </tr>
    </table>
        ...
        <tr>
            <td style="width: 48%; padding-right: 2%;">
                <div class="info-box">
                    ...
                    <strong style="font-size: 13px;">{{ $purchaseOrder->supplier->name ?? 'Público General' }}</strong><br>
                    <span style="font-size: 11px;">RUC/DNI: {{ $purchaseOrder->supplier->document_number ?? '—' }}</span><br>
                    <span style="font-size: 11px;">{{ $purchaseOrder->supplier->address ?? '' }}</span>
                </div>
            </td>
            <td style="width: 48%; padding-left: 2%;">
                <div class="info-box">
                    <span class="info-title">Detalles de Entrega</span>
                    <strong>Almacén:</strong> {{ $purchaseOrder->warehouse->name ?? 'Principal' }}<br>
                    <strong>Obs:</strong> {{ $purchaseOrder->observation ?? 'Ninguna' }}
                </div>
            </td>
        </tr>
    </table>
    ...
    <div style="width: 100%; clear: both;">
        <table class="totals-table">
            <tr>
                ...
                <td class="text-right">S/ {{ number_format($purchaseOrder->total, 2) }}</td>
            </tr>
            <tr class="total-row">
