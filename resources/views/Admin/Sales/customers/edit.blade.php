<x-admin-layout title="Editar cliente | Inventarios" :breadcrumbs="[
    [
        'name' => 'Dashboard',
    ...
    <x-w-card>
        <form class="space-y-4" method="POST" action="{{ route('admin.customers.update', $customer) }}">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <x-w-native-select label="Tipo Doc" name="identity_id">
                    @foreach ($identities as $item)
                        <option value="{{ $item->id }}" @selected(old('identity_id', $customer->identity_id) == $item->id)>{{ $item->name }}</option>
                    @endforeach
                </x-w-native-select>

                <x-w-input label="Numéro del documento" name="document_number"
                    placeholder="Numéro del documento del cliente"
